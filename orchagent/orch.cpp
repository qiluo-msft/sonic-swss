...
void Orch::addConsumer(DBConnector *db, string tableName)
{
    ...
    {
        addConsumer(tableName, new Consumer(new SubscriberStateTable(db, tableName), this));
    }
    else
    {
        addConsumer(tableName, new Consumer(new ConsumerStateTable(db, tableName, gBatchSize), this));
    }
}

void Orch::addConsumer(string tableName, ExecutableSelectable* consumer)
{
    m_consumerMap.emplace(std::piecewise_construct,
            std::forward_as_tuple(tableName),
            std::forward_as_tuple(consumer));
}
